...
export const taskCreate = async (req, res, next) => {
  ...
  try {
    ...
    if (Object.keys(fileList)?.length > 0) {
      ...
      dldName = randomKey() + req?.files?.thumbnail?.name;
      coverName = randomKey() + req?.files?.thumbnail?.name;
      ...
      const [imageSave,dldSave,coverSave,gallary1Save,gallary2Save,gallary3Save,] = await Promise.all([
        fileUpload(imgName, req.files?.thumbnail),
        fileUpload(dldName, req.files?.thumbnail),
        fileUpload(coverName, req.files?.thumbnail),
      ...
      if (
        ...
        return next(
          createError(400, "Filied to upload profile please try agin")
        );
    }
...
export const updateTask = async (req, res, next) => {
  ...
  try {
    ...
    if (todo.user_id.toString() !== req.user.id.toString()) {
      return next(
        createError(400, "You are not authorized to update this task")
      );
    }

    taskUpdate = await taskModel.findByIdAndUpdate(
      req.params.id,
      {
        $set: {
          ...req.body,
...
export const deleteTask = async (req, res, next) => {
  try {
    ...
    res
      .status(200)
      .json({ sucess: true, message: "Deleted project successfully" });
  } catch (err) {
    next(createError(400, err.message));
  }
};
